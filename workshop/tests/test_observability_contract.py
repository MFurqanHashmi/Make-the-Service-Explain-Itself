...
class ContractTests(unittest.TestCase):
    def test_checkpoint_markers_and_cumulative_states(self):
        ...
        for checkpoint in checkpoints:
            for name in ("checkout.py", "validation.py"):
                ast.parse((ROOT / "workshop/checkpoints" / checkpoint / name).read_text())
        self.assertNotIn("checkout_completed.add", (ROOT / "workshop/checkpoints/0-starter/checkout.py").read_text())
        self.assertIn("checkout_completed.add", (ROOT / "workshop/checkpoints/1-metrics-complete/checkout.py").read_text())
        trace_code = (ROOT / "workshop/checkpoints/2-traces-complete/validation.py").read_text()
        ...
        log_code = (ROOT / "workshop/checkpoints/3-logs-complete/validation.py").read_text()
    ...
    def test_no_sensitive_or_high_cardinality_metric_labels(self):
        code = (ROOT / "workshop/checkpoints/1-metrics-complete/checkout.py").read_text()
    ...
    def test_dashboard_uses_pinned_datasource(self):
        dashboard = json.loads((ROOT / "workshop/telemetry/grafana/dashboards/checkout-overview.json").read_text())
