...
│   ├── grafana/
│   └── otel/
├── lab                  # executable participant command
├── scripts/
│   ├── check_metrics.py
│   ├── check_traces.py
│   ├── check_logs.py
...
The root `./lab` command and the `scripts/` helper directory must remain separate because a file and directory cannot share the same path. The participant guide must always identify one repository-relative path and one insertion marker per edit.
