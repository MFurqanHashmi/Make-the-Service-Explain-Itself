...
Native validation found and corrected grouped-counter undercounting for series first created during the traffic window. `scripts/check_metrics.py` therefore uses the last sample before the traffic boundary, or zero if the series did not yet exist, and handles counter resets after hot reload.
